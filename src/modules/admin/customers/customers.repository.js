...
export class AdminCustomersRepository {
  async findAll({ offset, limit, search, status, sortBy = 'created_at', sortOrder = 'DESC' }) {
    ...
    const clauses = [`(u.role = 'CUSTOMER' OR EXISTS (SELECT 1 FROM orders eo WHERE eo.customer_id = u.id))`]
    let idx = 1
    ...
    const { rows } = await query(
      `SELECT u.id, u.name, u.phone, u.email, u.avatar_url, u.is_active, u.is_blocked, u.created_at,
       ...
       LEFT JOIN (
         SELECT customer_id, COUNT(*)::int AS order_count, SUM(total_payable) AS total_spent, MAX(created_at) AS last_order_at
         FROM orders WHERE status != 'CANCELLED' GROUP BY customer_id
       ) o_stats ON o_stats.customer_id = u.id
       WHERE ${where}
  ...
  async findById(id) {
    const { rows: [customer] } = await query(
      `SELECT u.*, COALESCE(w.balance, 0) AS wallet_balance,
       ...
       LEFT JOIN wallets w ON w.user_id = u.id
       LEFT JOIN (
         SELECT customer_id, COUNT(*)::int AS order_count, SUM(total_payable) AS total_spent,
                AVG(total_payable) AS avg_order, MAX(created_at) AS last_order_at
         FROM orders WHERE status != 'CANCELLED' GROUP BY customer_id
       ) o_stats ON o_stats.customer_id = u.id
       LEFT JOIN (
       ...
       ) o_status ON o_status.customer_id = u.id
       WHERE u.id = $1 AND (u.role = 'CUSTOMER' OR EXISTS (SELECT 1 FROM orders eo WHERE eo.customer_id = u.id))`,
      [id]
    )
  ...
  async getCustomerOrders(customerId, { offset, limit }) {
    const { rows } = await query(
      `SELECT id, order_number, status, total_payable, payment_method, created_at
       FROM orders WHERE customer_id = $1
       ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
      [customerId, limit, offset]
    )
    const countRes = await query('SELECT COUNT(*)::int AS total FROM orders WHERE customer_id = $1', [customerId])
    const total = countRes.rows[0].total
    const page = Math.floor(offset / limit) + 1
    return {
      orders: rows,
  ...
  async getLTV() {
    const { rows } = await query(
      `SELECT u.id, u.name, u.phone, u.email, u.created_at,
       ...
       FROM users u
       LEFT JOIN (
         SELECT customer_id, SUM(total_payable) AS total_spent, COUNT(*)::int AS order_count, AVG(total_payable) AS avg_order
         FROM orders WHERE status = 'DELIVERED' GROUP BY customer_id
       ) o ON o.customer_id = u.id
       WHERE (u.role = 'CUSTOMER' OR EXISTS (SELECT 1 FROM orders eo WHERE eo.customer_id = u.id))
  ...
  async getChurned(days = 30) {
    const { rows } = await query(
      `SELECT u.id, u.name, u.phone, u.email,
       ...
       FROM users u
       JOIN (
         SELECT customer_id, MAX(created_at) AS last_order_at, COUNT(*)::int AS order_count, SUM(total_payable) AS total_spent
         FROM orders WHERE status = 'DELIVERED' GROUP BY customer_id HAVING COUNT(*) >= 2
       ) o ON o.customer_id = u.id
       WHERE (u.role = 'CUSTOMER' OR EXISTS (SELECT 1 FROM orders eo WHERE eo.customer_id = u.id)) AND u.is_active = true
    ...
    const { rows } = await query(
      `SELECT u.id, u.name, u.phone, u.email, u.loyalty_points,
       ...
       JOIN (
         SELECT customer_id, COUNT(*)::int AS order_count, SUM(total_payable) AS total_spent,
                AVG(total_payable) AS avg_order, MAX(created_at) AS last_order_at
         FROM orders WHERE status = 'DELIVERED' GROUP BY customer_id HAVING COUNT(*) >= $1
       ) o ON o.customer_id = u.id
       WHERE (u.role = 'CUSTOMER' OR EXISTS (SELECT 1 FROM orders eo WHERE eo.customer_id = u.id)) AND u.is_active = true
       ORDER BY o.total_spent DESC`,
      [minOrders]
    )
    return rows.map(r => ({ ...r, total_spent: parseFloat(r.total_spent), avg_order_value: parseFloat(r.avg_order_value), wallet_balance: parseFloat(r.wallet_balance) }))
  }
